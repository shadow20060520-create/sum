...
-- 模拟 Supabase 内置的 auth.users 表 (仅供外部查看 DDL 关系使用，实际在 Supabase 中已存在)
...
-- 4. Auth 注册同步触发器 (核心逻辑)
-- 设计意图: 当 Supabase auth.users 新增记录时，自动拉起 profiles 数据。
...
CREATE TABLE public.student_records (
    ...
    attempt_num INTEGER DEFAULT 1,           -- 记录跟读次数，支持保留历史记录 (解答你关于练习次数的疑问：我们保留历史，通过 attempt_num 区分)
...
-- GIN 索引: 榨干 JSONB 的查询性能 (如未来需要检索特定错音)
-- 支持操作符 @>, ?, ?&, ?| 
